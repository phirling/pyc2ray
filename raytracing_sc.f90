module raytracing_sc
    ...
    real(real64), parameter :: pi = 3.14159265358979323846264338_real64 ! Double precision pi
    ...
    subroutine evolve2D(dt,k,srcpos,ns,niter,last_l,last_r,coldensh_out,sig,dr,ndens,NumSrc,m1,m2,m3)
        ...
        integer,intent(in) :: srcpos(3,NumSrc)      ! positions of ALL sources (mesh)
        real(kind=real64), intent(in) :: ndens(m1,m2,m3)      ! Hydrogen Density Field
        !real(kind=real64), intent(in) :: ndens_p    ! Local density of cell
        real(kind=real64), dimension(3), intent(in) :: dr       ! Cell size
        real(kind=real64),intent(inout) :: coldensh_out(m1,m2,m3) !< Outgoing column density of the cells
        real(kind=real64),intent(in):: sig !< Hydrogen ionization cross section (sigma_HI_at_ion_freq)
        integer, intent(in) :: m1 ! < mesh size x (hidden by f2py)
    ...
    subroutine evolve0D(dt,rtpos,srcpos,ns,niter,coldensh_out,sig,dr,ndens,NumSrc,m1,m2,m3)
        ...
        real(kind=real64),dimension(0:1) :: coldenshe_in,coldenshe_out_temp
        logical :: stop_rad_transfer
        real(kind=real64) :: ndens_p    ! Local density of cell
        

        ! type(photrates) :: phi, dummiphi  ----> Replace by array
        ! type(ionstates) :: ion            ----> Replace by array
        ...
        if (coldensh_out(pos(1),pos(2),pos(3)) == 0.0) then
            ...
            ! Original: compiler hint ALLFRAC (??) anyway, basically variable ion is set to "close to zero"
            ! --> #ifdef ALLFRAC       
            ! -->     do nx=0,1
            ...
            coldensh_out(pos(1),pos(2),pos(3))=coldensh_in + ndens_p*path ! coldens(path,1.0_real64,ndens_p) ! TODO: replace 1.0 by actual xh
    ...
    ! ===============================================================================================
    ! CINTERP Routine to do the actual short characteristics
    ! ===============================================================================================
    ...
    !           real(kind=real64) function coldens(path,xh0,ndens)
    !               use, intrinsic :: iso_fortran_env, only: real64
    !               ! Calculates the column density (of hydrogen)
    !               ! for a cell of ionization fraction xh, length dr, 
    !               ! and density ndens
    !               real(kind=real64),intent(in) :: path !< path length through cell
    !               real(kind=real64),intent(in) :: xh0  !< neutral H fraction
    !               real(kind=real64),intent(in) :: ndens !< number density of H
