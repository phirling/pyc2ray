...
class zTomography_xfrac_3panels:
    def __init__(self,datacube1,datacube2,datacube3,zi,incr=10,xmin=None,cmap='jet',fs=6):
        self.data1 = datacube1
        self.data2 = datacube2
        self.data3 = datacube3
        self.incr = incr
        ...
        self.im1 = self.ax1.imshow(1.0-self.data1[:,:,zi],origin='lower',norm='log',cmap=self.cmap,vmax=1.0,vmin=vmin)
        self.cb1 = plt.colorbar(self.im1,ax=self.ax1)
        self.cb1.set_label(label=r"$\log \Gamma_1$ [s$^{-1}$]",size=15)

        # Panel 2
        self.ax2.set_title(f"Neutral Hydrogen Fraction, pyC2Ray",fontsize=12)
        self.im2 = self.ax2.imshow(1.0-self.data2[:,:,zi],origin='lower',norm='log',cmap=self.cmap,vmax=1.0,vmin=vmin)
        self.cb2 = plt.colorbar(self.im2,ax=self.ax2)
        self.cb2.set_label(label=r"$\log \Gamma_2$ [s$^{-1}$]",size=15)
    ...
    def switch(self,event):
        ...
        if up or down:
            if zz in range(self.N):
                self.im1.set_data(1.0 - self.data1[:,:,zz])
                self.im2.set_data(1.0 - self.data2[:,:,zz])
                self.im3.set_data(self.data3[:,:,zz])
                self.zz = zz
                self.fig.canvas.draw()