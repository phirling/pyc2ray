...
zslice  = 50       # z-coordinate of box to visualize
...
# Metrics
def residual(A,B):
    A_m = A #np.where(in_sphere,A,1)
    B_m = B #np.where(in_sphere,B,1)
...
print("Running OCTA...")
t3 = time.time()
octa.octa_multiple(srcpos,srcflux,rad,cdh1,sig,dxbox,ndens,xh_av,phi_ion1,numsrc,N)
t4 = time.time()
...
if plot:
    ...
    slice1 = np.log(phi_ion_f[:,:,zslice])
    im1 = ax[0,0].imshow(slice1,origin='lower') #,vmin=min_cdh,vmax=max_cdh)
    ...
    resid1 = residual(phi_ion_f, phi_ion_f)
